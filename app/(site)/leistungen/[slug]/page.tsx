...
export const generateMetadata = async ({ params }: { params: { slug: string } }) => {
  const entry = getContentBySlug("leistungen", params.slug);
  if (!entry) {
    return {};
  }
...
export default async function LeistungenPage({ params }: { params: { slug: string } }) {
  const entry = getContentBySlug("leistungen", params.slug);
  if (!entry) {
    notFound();
  }
  ...
  const schemas = [
    ...
    buildServiceSchema({ name: entry.title, description: entry.excerpt, url: canonical })
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      />
      <Breadcrumbs
        items={[{ label: "Startseite", href: "/" }, { label: entry.title, href: `/leistungen/${entry.slug}` }]}
      />
